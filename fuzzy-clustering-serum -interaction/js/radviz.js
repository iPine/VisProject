...
var radvizComponent = function() {
    ...
    var render = function(data) {
        ...
        data.forEach(function(d, i) {
            ...
            if(d_flag){
                // if(iB==maxIndex){
                    if(d[dB]>=interval[0] && d[dB] <=interval[1]){
                        d[dB] = d[dB];
...
//jquery控制brush       
          ...
          d3.select('svg').selectAll('circle.dot')
            ...
            classes.forEach(function(c,j){
                ...
                points.forEach(function(d){
                    if(interv == 10){ bar[parseInt(d[c]/0.1)].value++;}
                    if(interv == 20){ bar[parseInt(d[c]/0.05)].value++;}
                    if(interv == 5){ bar[parseInt(d[c]/0.2)].value++;}
        ...
        if (config.useRepulsion) {
         ...
         // if(cNum == 13){order = [5, 4, 6, 13, 3, 8, 2, 12, 10, 9, 7, 1, 11];}
                    var order1 = [];
                    if(flag){
                        if(cNum == 12){order1 = [11, 1, 7, 0, 9, 10, 8, 5, 4, 3, 6, 2];}
                        if(cNum == 13){order1 = [11, 6, 4, 1, 0, 2, 10, 5, 9, 8, 12, 7, 3];}
...
//jquery控制chord
        ...
        var colorAnchor = d3.scale.ordinal().domain(['C1','C2','C3','C4','C5','C6','C7','C8','C9','C10','C11','C12',"C13"]).range(['#FF4500', '#de3669', '#00D998', 'teal', '#00CD00','#f2cc03', '#9400D3', '#b58453', '#e3701e', '#F07484','#FFCEA6', '#bfbfbf','#B32748']);
